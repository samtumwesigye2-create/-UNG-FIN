...
E=create_engine(DB,pool_pre_ping=True) if DB else None
...
def put(r):
    if E:
        with E.begin() as c:
            c.execute(text('INSERT INTO records(id,payload) VALUES(:id,CAST(:p AS JSONB)) ON CONFLICT(id) DO UPDATE SET payload=EXCLUDED.payload'),{'id':r['id'],'p':json.dumps(r)})
    return r
