...
SYSTEM_ID='UNG-MIDAS'; LEGACY_ID='UNG-FIN'; VERSION='0.4.0'
...
def startup():
 init_db(); init_accounting(); init_ap(); init_midas_events()
@app.get('/')
...
@app.get('/v1/system')
def system(): return {'system_id':SYSTEM_ID,'legacy_id':LEGACY_ID,'domain':'finance','dependencies':dependencies(),'capabilities':['ledger','nexus-procurement-handoff','supplier-invoices','accounts-payable','vendor-open-items','vendor-cleared-items','double-entry-accounting','immutable-accounting-history','tax-withholding-accounting','procure-match-consumer','payment-eligibility','payment-status','persistent-outbox','idempotent-finance-events']}
@app.get('/v1/ledger')
